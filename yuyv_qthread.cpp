...
void YUYVQThread::run(){
    /* 打开设备 */
    v4l2_fd = -1;
    v4l2_fd = open(dev_name.toUtf8().data(), O_RDWR | O_NONBLOCK, 0);
    if(v4l2_fd < 0){
	...
	if (-1  == ioctl(v4l2_fd, VIDIOC_G_FMT, &fmt)) {
    ...
    while(show_flag){
        ...
        QImage img = QImage(onebuf,PIXWIDTH,PIXHEIGHT,QImage::Format_RGB888);
        emit sign_img(img);
		usleep(40000);
    }
}
